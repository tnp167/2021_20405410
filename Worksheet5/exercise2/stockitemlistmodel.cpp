...
void StockItemListModel::removeItem( const QModelIndex & index ) {
		emit beginRemoveRows( QModelIndex(), stockItems.size()-1, stockItems.size()-1 );
...
double StockItemListModel::getItemUnitCost(int i){
	return stockItems[i].getUnitCost();
}

int StockItemListModel::getItemStockLevel(int i){
	return stockItems[i].getStockLevel();
}

bool StockItemListModel::getItemReorder(int i){
	return stockItems[i].getReorder();
}
// ---------------------------------------------------------------------		
